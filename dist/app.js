...
let mode='add',pendingDelete=null,animationTimer,animationImpactTimer,animationFinishTimer,toastTimer,captionTimer,hitCompleteHandler=null,hitInProgress=false,armAnimation=null,switching=false,queuedPig=null,transitionRun=null,hitSpeed='slow';
...
function stopAnimation(){cancelTransition();cancelAnimationFrame(animationTimer);clearTimeout(animationImpactTimer);clearTimeout(animationFinishTimer);armAnimation?.cancel();armAnimation=null;hitCompleteHandler=null;$('world').classList.remove('striking','dizzy','impacted','fast-hit');$('effects').replaceChildren();hitInProgress=false;$('hit').disabled=!current()}
...
function finishHit(){if(!hitInProgress)return;const handler=hitCompleteHandler;stopAnimation();if(handler){Promise.resolve(handler()).catch(()=>notify('联机数据暂时没有更新'));return}const p=current();if(p&&p.count<Number.MAX_SAFE_INTEGER){p.count++;save();render()}}
function showImpactCaption(fast){
  ...
  const pop=$('pop');
  pop.textContent=captions[Math.floor(Math.random()*captions.length)];
  pop.classList.remove('caption-pop');
  void pop.offsetWidth;
  pop.classList.add('caption-pop');
  clearTimeout(captionTimer);
  captionTimer=setTimeout(()=>{pop.classList.remove('caption-pop');pop.textContent='+1'},fast?460:900);
}
function hit(){
  const p=current();
  ...
  armAnimation.finished.then(finishHit).catch(()=>{});
  const impact=()=>{
    if(!hitInProgress||world.classList.contains('impacted'))return;
  ...
  const step=()=>{
    ...
    if(time>=contact-.1)impact();
    if(time>=duration){finishHit();return}
    animationTimer=requestAnimationFrame(step);
  };
  animationTimer=requestAnimationFrame(step);
  // The visual trigger matches single-player; timers only guarantee cleanup if frames are paused.
  animationImpactTimer=setTimeout(impact,contact);
...
document.addEventListener('keydown',e=>{if(e.code==='Space'&&!e.repeat&&!['INPUT','TEXTAREA','BUTTON','SELECT'].includes(document.activeElement.tagName)&&!$('editor').open&&!$('delete-dialog').open){e.preventDefault();window.PiggyOnline?.isActive?.()?window.PiggyOnline.hit():hit()}});
function applySky(night,persist=true){document.body.classList.toggle('night',night);$('sky-toggle').setAttribute('aria-pressed',String(night));$('sky-toggle').title=night?'切换为白天':'切换为黑夜';$('sky-toggle').querySelector('.sun-icon').textContent=night?'☾':'☀';$('sky-toggle').querySelector('.toggle-label').textContent=night?'黑夜':'白天';$('night-sky').replaceChildren();if(night){for(let i=0;i<28;i++){const star=document.createElement('i');star.className='sky-star';star.style.left=(Math.random()*100)+'%';star.style.top=(Math.random()*80)+'%';star.style.animationDelay=(Math.random()*1.8)+'s';star.style.setProperty('--size',(1+Math.random()*3)+'px');$('night-sky').append(star)}if(Math.random()<.3){const moon=document.createElement('i');moon.className='moon';moon.setAttribute('aria-label','月亮');$('night-sky').append(moon)}}if(persist){try{localStorage.setItem(KEY+'-sky',night?'night':'day')}catch{}queueCloudSync?.()}}
function setSky(night){if(window.PiggyOnline?.isActive?.()){window.PiggyOnline.setViewerSky(night);return}applySky(night)}
...
async function transitionOnlineScene(apply){
  if(switching)return false;
  stopAnimation();
  const scene=$('island-scene'),ghost=scene.cloneNode(true);
  ghost.removeAttribute('id');ghost.querySelectorAll('[id]').forEach(el=>el.removeAttribute('id'));
  ghost.querySelectorAll('button').forEach(el=>{el.disabled=true;el.tabIndex=-1});
  ghost.classList.add('departing-scene');ghost.setAttribute('aria-hidden','true');scene.before(ghost);
  switching=true;apply();lockSceneControls(true);
  const reduced=matchMedia('(prefers-reduced-motion: reduce)').matches,duration=reduced?120:800;
  const outgoing=ghost.animate(reduced?[{opacity:1},{opacity:0}]:[{transform:'translate(0,0) scale(1)',opacity:1},{transform:'translate(-112%,-4%) scale(.76)',opacity:0}],{duration,easing:'cubic-bezier(.4,0,.6,1)',fill:'both'});
  const incoming=scene.animate(reduced?[{opacity:0},{opacity:1}]:[{transform:'translate(112%,-4%) scale(.76)',opacity:0},{transform:'translate(0,0) scale(1)',opacity:1}],{duration,easing:'cubic-bezier(.22,.7,.25,1)',fill:'both'});
  const run={ghost,animations:[outgoing,incoming]};transitionRun=run;
  await Promise.allSettled(run.animations.map(a=>a.finished));
  if(transitionRun===run)cancelTransition();
  return true;
}
window.PiggyGame={playOnlineHit,transitionOnlineScene,applySky,renderSingle:render,singleHit:hit,notify};
...
$('auth-form').onsubmit=async event=>{event.preventDefault();const username=$('auth-username').value.trim(),password=$('auth-password').value,confirm=$('auth-confirm').value;$('auth-error').textContent='';if(!validUsername(username))return void($('auth-error').textContent='用户名只能是 1–10 位英文字母。');if(!validPassword(password))return void($('auth-error').textContent='密码只能是至少 8 位数字。');if(cloud.mode==='signup'&&password!==confirm)return void($('auth-error').textContent='两次输入的密码不一致。');const submit=$('auth-submit');submit.disabled=true;submit.textContent=cloud.mode==='signup'?'正在创建…':'正在登录…';try{const result=await invokeAuth(cloud.mode==='signup'?'auth-signup':'auth-login',{username,password});const {error}=await cloud.client.auth.setSession({access_token:result.session.access_token,refresh_token:result.session.refresh_token});if(error)throw error;cloud.player=(await cloud.client.auth.getUser()).data.user;if(cloud.mode==='signup'){await cloud.client.from('game_progress').update(cloudPayload()).eq('player_id',cloud.player.id)}await loadCloudProgress();$('auth-dialog').close();window.dispatchEvent(new CustomEvent('piggy-authenticated',{detail:cloud.player}));notify('已读取云存档')}catch(error){$('auth-error').textContent=authError(error.code||error.message)}finally{submit.disabled=false;submit.textContent=cloud.mode==='signup'?'创建账号并进入游戏':'登录并读取云存档'}};
