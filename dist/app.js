...
function validPassword(value){return /^\d{4,8}$/.test(value)}
function cloudPayload(){
  const pigs=state.pigs.map(p=>({id:p.id,name:p.name,count:p.count}));
  return {piggies:pigs,selected_pig_id:state.selected||'',hits:pigs.reduce((sum,p)=>sum+p.count,0),coins:0,level:1,xp:0,upgrades:[],unlocked_content:[],achievements:[],inventory:[],purchased_items:[],settings:{sky:document.body.classList.contains('night')?'night':'day'},statistics:{total_hits:pigs.reduce((sum,p)=>sum+p.count,0)}}
}
...
async function loadCloudProgress(){
  ...
  try{localStorage.setItem(KEY,JSON.stringify(state));localStorage.setItem(KEY+'-sky',data.settings?.sky==='night'?'night':'day')}catch{}
  setSky(data.settings?.sky==='night');render();
}
...
function authError(code){return ({invalid_username:'用户名只能是 1–10 位英文字母。',invalid_password:'密码只能是 4–8 位数字。',username_taken:'这个用户名已被使用。',account_not_found:'Account not found.',incorrect_password:'Incorrect password.',rate_limited:'尝试次数过多，请 10 分钟后再试。',signup_failed:'账号创建失败，请稍后重试。',network_error:'无法连接云存档，请检查网络。'})[code]||'操作失败，请稍后重试。'}
